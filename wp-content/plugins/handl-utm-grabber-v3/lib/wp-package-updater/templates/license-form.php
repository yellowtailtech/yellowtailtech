...
<div class="wrap-license" data-package_slug="<?php echo esc_attr( $package_slug ); ?>" id="<?php echo esc_attr( 'wrap_license_' . $package_id ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'license_nonce' ) ); ?>">
	...
	<p>
		<label style="vertical-align: initial;"><?php esc_html_e( 'License key to change:', 'wp-package-updater' ); ?></label>
        <input class="regular-text license" type="text" id="<?php echo esc_attr( 'license_key_' . $package_id ); ?>" value="<?php echo esc_html( $license ); ?>" <?php echo empty($license) ? '' : esc_attr('readonly'); ?> >
		<input type="button" value="Activate" class="button-primary activate-license" <?php echo ( $show_license ) ? esc_attr( 'disabled' ) : ''; ?>/>
		<input type="button" value="Deactivate" class="button deactivate-license" <?php echo ( !$show_license ) ? esc_attr( 'disabled' ) : ''; ?>/>
	</p>
</div>
