...
class WC_Report_Source_By_Date extends WC_Admin_Report {
	...
	public function output_report() {
		...
		$this->utm_type = ! empty( $_GET['utm_type'] ) ? sanitize_text_field( wp_unslash( $_GET['utm_type'] ) ) : 'utm_source';
		if ( ! in_array( $current_range, array( 'utm_source', 'utm_medium', 'utm_campaign' ), true ) ) {
			$this->utm_type = 'utm_source';
		}

		$this->utm_value = ! empty( $_GET['utm_value'] ) ? sanitize_text_field( wp_unslash( $_GET['utm_value'] ) ) : '';

		include dirname( __FILE__ ) . '/views/html-source-by-date.php';
	}
	...
	private function query_report_data() {
		...
		$meta_key = $this->utm_type  != '' ? $this->utm_type : 'utm_source';
        $meta_value = $this->utm_value;
        $meta_operator = $meta_value != '' ? '=' : '!=';
		...
		$this->report_data->order_counts = (array) $this->get_order_report_data(
			array(
				...
				'group_by'     => $this->group_by_query.", meta_$meta_key.meta_value",
