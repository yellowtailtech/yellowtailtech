...
use \OTGS\Toolset\Views\Controller\Filters\Post\ProductPrice\ProductPriceGui;

/**
 * Save the post product on sale query filter.
 */
class WPV_Ajax_Handler_Filter_Post_Product_Price_Update extends \Toolset_Ajax_Handler_Abstract {
	...
	public function process_call( $arguments ) {
		...
		parse_str( $filter_options, $filter_product_price );
		...
		try {
			...
			$filter_settings[ ProductPrice::SLUG ] = array(
				'mode' => toolset_getnest( $filter_product_price, array( ProductPrice::SLUG, 'mode' ), 'query_filter' ),
				'values' => array(
					'min' => toolset_getnest( $filter_product_price, array( ProductPrice::SLUG, 'values', 'min' ), '' ),
					'max' => toolset_getnest( $filter_product_price, array( ProductPrice::SLUG, 'values', 'max' ), '' ),
				),
			);
		...
		$data = array(
			...
			'summary' => \WPV_Filter_Manager::get_instance()
				->get_filter( \Toolset_Element_Domain::POSTS, ProductPrice::SLUG )
				->get_gui()
