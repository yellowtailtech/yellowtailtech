...
class Manager extends ManagerBase {
	...
	/**
	 * Register API hooks to get, set and delete the cache for termmeta fields.
	 *
	 * @since 2.8.1
	 */
	protected function add_hooks() {
		add_filter( 'wpv_get_visible_termmeta_cache', array( $this, 'get_or_generate_visible_cache' ), 10, 2 );
		add_action( 'wpv_generate_visible_termmeta_cache', array( $this, 'generate_visible_cache' ) );
		add_action( 'wpv_delete_visible_termmeta_cache', array( $this, 'delete_visible_cache' ) );

		add_filter( 'wpv_get_hidden_termmeta_cache', array( $this, 'get_or_generate_hidden_cache' ), 10, 2 );
		add_action( 'wpv_generate_hidden_termmeta_cache', array( $this, 'generate_hidden_cache' ) );
		add_action( 'wpv_delete_hidden_termmeta_cache', array( $this, 'delete_hidden_cache' ) );
	}

	/**
	 * Generate the query for visible termmeta fields.
	 *
	 * @param int $limit
	 * @return array
	 * @since 2.8.2
	...
	/**
	 * Generate the query for hidden termmeta fields.
	 *
	 * @param int $limit
	 * @return array
	 * @since 2.8.2
	...
	public function generate_hidden_query( $limit = null ) {
		...
		$meta_keys = $this->wpdb->get_col(
			$this->wpdb->prepare(
				"SELECT DISTINCT meta_key FROM {$this->wpdb->termmeta}
				WHERE LEFT(meta_key, 1) == '_'
				{$tmf_mulsitise_string}
				LIMIT %d",
				$values_to_prepare
