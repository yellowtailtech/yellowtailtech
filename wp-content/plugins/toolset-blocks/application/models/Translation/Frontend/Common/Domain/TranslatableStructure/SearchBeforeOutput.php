...
/**
 * Class SearchBeforeOutput
 *
 * @package OTGS\Toolset\Views\Models\Translation\Frontend\Common\Domain
 *
 * @since TB 1.3
 */
class SearchBeforeOutput implements ITranslatable {
	...
	public function translate_content( PostContent $post_untranslated, IBlockContent $block_translated ) {
		...
		// Get start position, if not possible continue with next structure.
		...
		if( $pos_start > $pos_search || $pos_search > $pos_loop ) {
		...
		// Between Block Start and Search.
		$translation = $block_translated->get_content_between_start_and_search();
		$post_untranslated->apply_translation_between_start_and_search( $translation );

		// Between Search and Loop.
		$translation = $block_translated->get_content_between_search_and_output();
		$post_untranslated->apply_translation_between_search_and_loop( $translation );
