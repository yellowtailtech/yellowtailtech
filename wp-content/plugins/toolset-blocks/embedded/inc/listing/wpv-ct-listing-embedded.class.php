...
class WPV_CT_List_Table_Embedded extends WPV_List_Table_Embedded {
    ...
    public function column_used_on( $item ) {
        ...
        if( $item->is_owned_by_view ) {
			...
			// "single posts with conditions"
			$assigned_single_pts = $item->get_assigned_single_post_types_with_conditions();
			foreach ( $assigned_single_pts as $loop ) {
				/** @var UsagePostType $ct_usage_setting */
				$ct_usage_setting = $loop['single_ct_conditions'];
				$conditions_text = $ct_usage_setting->toString();
				$loop_text = sprintf( '%s ( %s ) %s', $loop['display_name'], $conditions_text, __( ' (single)', 'wpv-views' ) );
				$list[] = sprintf( '<li>%s</li>', $loop_text );
			}
