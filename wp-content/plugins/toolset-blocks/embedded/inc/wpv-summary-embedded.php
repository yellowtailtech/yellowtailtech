...
function wpv_get_taxonomy_summary( $type, $view_settings, $category_selected ) {
	...
	$editor_summary = toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, 'taxonomy', $taxonomy, \WPV_Filter_Manager::EDITOR_SUMMARY ), false );
	if ( false !== $editor_summary ) {
		return '<span class="wpv-filter-multiple-summary-item">' . $editor_summary . '</span>';
	}

	$editor_mode = toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, 'taxonomy', $taxonomy, \WPV_Filter_Manager::EDITOR_MODE ), \WPV_Filter_Manager::FILTER_MODE_FULL );

	if ( \WPV_Filter_Manager::FILTER_MODE_FROM_SEARCH_FILTER === $editor_mode ) {
		return '<span class="wpv-filter-multiple-summary-item">'
			. sprintf( __( 'Filter based on the frontend search filter by %s.', 'wpv-views' ), '<strong>' . $taxonomy_name . '</strong>' )
			. '</span>';
	}

	if ( \WPV_Filter_Manager::FILTER_MODE_FROM_CONTENT_SELECTION === $editor_mode ) {
		return '<span class="wpv-filter-multiple-summary-item">'
			. sprintf( __( 'Filter by %s based on the current content selection.', 'wpv-views' ), '<strong>' . $taxonomy_name . '</strong>' )
			. '</span>';
	}
...
function wpv_get_filter_post_id_summary_txt( $view_settings, $short = false ) {
	$editor_summary = toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, 'post_id', \WPV_Filter_Manager::EDITOR_SUMMARY ), false );
	if ( false !== $editor_summary ) {
		return $editor_summary;
	}

	$editor_mode = toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, 'post_id', \WPV_Filter_Manager::EDITOR_MODE ), \WPV_Filter_Manager::FILTER_MODE_FULL );
	if ( \WPV_Filter_Manager::FILTER_MODE_FROM_CONTENT_SELECTION === $editor_mode ) {
		return __( 'Filter based on the current content selection.', 'wpv-views' );
	}
...
function wpv_get_filter_post_search_summary_txt( $view_settings, $short = false ) {
	$editor_summary = toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, 'post_search', \WPV_Filter_Manager::EDITOR_SUMMARY ), false );
	if ( false !== $editor_summary ) {
		return $editor_summary;
	}
	...
	$editor_mode = toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, 'post_search', \WPV_Filter_Manager::EDITOR_MODE ), \WPV_Filter_Manager::FILTER_MODE_FULL );
	if ( \WPV_Filter_Manager::FILTER_MODE_FROM_SEARCH_FILTER === $editor_mode ) {
		return sprintf( __( 'Filter %s based on the frontend input search filter.', 'wpv-views' ), $search_where );
	}
...
function wpv_get_filter_meta_field_summary_txt( $view_settings, $meta_type ) {
	...
	foreach ( array_keys( $view_settings ) as $key ) {
		if (
			...
			$result .= wpv_get_meta_field_summary( $name, $meta_type, $view_settings );
		}
	}
	return $result;
}
...
function wpv_get_meta_field_summary( $type, $meta_type, $view_settings = array() ) {
	global $WP_Views_fapi;
	...
	$editor_summary = toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, $field_domain, $field_name, \WPV_Filter_Manager::EDITOR_SUMMARY ), false );
	if ( false !== $editor_summary ) {
		return '<span class="wpv-filter-multiple-summary-item">' . $editor_summary . '</span>';
	}

	$editor_mode = toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, $field_domain, $field_name, \WPV_Filter_Manager::EDITOR_MODE ), \WPV_Filter_Manager::FILTER_MODE_FULL );
	if ( \WPV_Filter_Manager::FILTER_MODE_FROM_SEARCH_FILTER === $editor_mode ) {
		return '<span class="wpv-filter-multiple-summary-item">'
			. sprintf( __( 'Filter based on the frontend search filter by %s.', 'wpv-views' ), '<strong>' . $field_nicename . '</strong>' )
			. '</span>';
	}
