	...
	*/
	...
	static function wpv_add_filter_post_id_list_item( $view_settings ) {
		if ( isset( $view_settings['id_mode'][0] ) ) {
			...
			$filter_title = toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, self::SLUG, \WPV_Filter_Manager::EDITOR_TITLE ), __( 'Post ID filter', 'wpv-views' ) );
	...
	*/
	...
	static function wpv_get_list_item_ui_post_id( $view_settings = array() ) {
		...
		<p class='wpv-filter-post-id-edit-summary js-wpv-filter-summary js-wpv-filter-post-id-summary'>
			<?php
			echo wpv_get_filter_post_id_summary_txt( $view_settings );
			?>
		</p>
		<?php
		if ( \WPV_Filter_Manager::FILTER_MODE_FULL === toolset_getnest( $view_settings, array( \WPV_Filter_Manager::SETTING_KEY, 'post_id', \WPV_Filter_Manager::EDITOR_MODE ), \WPV_Filter_Manager::FILTER_MODE_FULL ) ) {
