<div class="js-toolset-theme-settings-single-option-wrap toolset-theme-settings-single-option-wrap <?php echo $target_css_class;?>" <?php echo $prepare_data_exclude;?> <?php echo $prepare_data_include;?>>
	...
	<div class="theme-option-switch-container js-theme-option-switch-container">
		...
		<label class="theme-option-switch-label" for="toolset-switch-<?php echo esc_attr( $element->name ); ?>-custom">
			...
			<input type="text" class="theme-option-switch-target-input js-theme-option-switch-target-input" data-types='<?php echo json_encode($element->type); ?>' name="<?php echo self::TOOLSET_CONTROL_PREFIX . esc_attr( $element->name ) ?>" value="<?php echo esc_attr($selected_value); ?>" id="<?php echo self::TOOLSET_CONTROL_PREFIX . esc_attr( $element->name ) ?>" <?php echo disabled( $switch_value, self::TOOLSET_SWITCH_DEFAULT, false ); ?> />
		</label>
	</div>
