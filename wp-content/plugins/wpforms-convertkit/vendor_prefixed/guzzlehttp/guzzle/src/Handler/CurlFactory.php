...
namespace WPFormsConvertKit\Vendor\GuzzleHttp\Handler;

use WPFormsConvertKit\Vendor\GuzzleHttp\Exception\ConnectException;
use WPFormsConvertKit\Vendor\GuzzleHttp\Exception\RequestException;
use WPFormsConvertKit\Vendor\GuzzleHttp\Promise as P;
use WPFormsConvertKit\Vendor\GuzzleHttp\Promise\FulfilledPromise;
use WPFormsConvertKit\Vendor\GuzzleHttp\Promise\PromiseInterface;
use WPFormsConvertKit\Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use WPFormsConvertKit\Vendor\GuzzleHttp\TransferStats;
use WPFormsConvertKit\Vendor\GuzzleHttp\Utils;
use WPFormsConvertKit\Vendor\Psr\Http\Message\RequestInterface;
use WPFormsConvertKit\Vendor\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \WPFormsConvertKit\Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \WPFormsConvertKit\Vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \WPFormsConvertKit\Vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \WPFormsConvertKit\Vendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
